...
from pytorch_tabnet.multiclass_utils import infer_multitask_output, infer_multitask_binary_output, check_output_dim
...
class TabNetMultiTaskBinaryClassifier(TabModel):
    ...
    def update_fit_params(self, X_train, y_train, eval_set, weights):
        output_dim, train_labels = infer_multitask_binary_output(y_train)
        #for _, y in eval_set:
        #    for task_idx in range(y.shape[1]):
        #        check_output_dim(train_labels[task_idx], y[:, task_idx])
        self.output_dim = output_dim
